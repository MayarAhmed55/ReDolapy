 ...
 <template>
  <div class="w-full">
    <div class="flex flex-col items-center text-center mb-5 sm:mb-6">
      <div class="flex items-center justify-center gap-2 sm:gap-3">
        <img :src="hangerIcon" alt="" class="w-7 h-7 sm:w-8 sm:h-8 shrink-0" aria-hidden="true" />
        <h2 class="text-lg sm:text-xl font-bold text-(--Primary-Text-color)">
          {{ titleLabel || $t('recycle.upload.title') }}
        </h2>
      </div>
      <p class="text-xs text-(--Secondary-Text-color) mt-1.5">
        {{ supportLabel || $t('recycle.upload.support_text') }}
      </p>
    </div>
    ...
    >
      ...
      <div class="flex flex-col items-center gap-2 pointer-events-none px-4">
        <img :src="uploadIcon" alt="" class="w-10 h-10 sm:w-11 sm:h-11" aria-hidden="true" />
        <p class="text-sm text-(--Disabled-Text-color)">
          {{ dragPromptLabel || $t('recycle.upload.drag_prompt') }}
        </p>
      </div>
    </div>
    ...
    <div v-if="files.length" class="flex flex-wrap justify-start gap-4 sm:gap-5 mt-5 sm:mt-6">
      ...
      >
        <div class="rounded-2xl bg-[#D6E8F5] p-2 sm:p-2.5 overflow-hidden">
          <img
            :src="f.url"
            :alt="`${$t('recycle.upload.piece_prefix')} ${i + 1}`"
            class="w-full h-28 sm:h-32 object-cover rounded-xl block"
          />
          <span class="absolute bottom-4 start-4 bg-white/95 text-[10px] sm:text-xs font-medium text-(--Primary-Text-color) px-2 py-0.5 rounded-md shadow-sm">
            {{ $t('recycle.upload.piece_prefix') }} {{ i + 1 }}
          </span>
        </div>
        <button
          type="button"
    ...
    <div class="flex justify-center mt-5 sm:mt-6">
      <button
        v-if="files.length"
        type="button"
        class="recycle-action-btn recycle-action-btn--compact animate-fade-in"
        :disabled="analyzing"
        @click="$emit('analyze')"
      >
        <img :src="starsIcon" alt="" aria-hidden="true" />
        {{ analyzing ? (loadingLabel || $t('recycle.upload.loading')) : (buttonLabel || $t('recycle.upload.button')) }}
      </button>
    </div>
...
export default {
  ...
  props: {
    ...
    analyzing:        { type: Boolean, default: false },
    error:            { type: String, default: '' },
    // 💡 Declared structural configuration props sent down by the parent view
    titleLabel:       { type: String, default: '' },
    supportLabel:     { type: String, default: '' },
    dragPromptLabel:  { type: String, default: '' },
  ...
  methods: {
    ...
    add(incoming) {
      ...
      if (rejected.length) {
        // 💡 Runtime formatting warning fallback
